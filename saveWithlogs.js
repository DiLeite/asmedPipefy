...
const LOG_DIR = "./logs"; // Diretório para armazenar logs de erro

// Garante que a pasta de logs existe
fs.ensureDirSync(LOG_DIR);
const logFilePath = path.join(LOG_DIR, `errors_${Date.now()}.log`);
...
async function downloadFile(url, filename, outputPath) {
    ...
    try {
        ...
        logError(`❌ Erro ao baixar ${url}: ${error.response ? error.response.statusText : error.message}`);
        if (error.response && error.response.status === 401) {
            logError("🔒 Verifique os cookies.");
        }
    }
}
...
async function processJson(jsonFile) {
    try {
        const jsonFileName = path.basename(jsonFile, ".json");
        const jsonFolderPath = path.join(BASE_PATH, jsonFileName);
        ...
        logError(`❌ Erro ao processar ${jsonFile}: ${error.message}`);
    }
}
...
function zipFolder(folderPath) {
    return new Promise((resolve, reject) => {
        ...
        exec(`node zip.js "${folderPath}"`, (error, stdout, stderr) => {
            if (error) {
                logError(`❌ Erro ao compactar ${folderPath}: ${error.message}`);
                reject(error);
                return;
            }
            if (stderr) {
                logError(`⚠ Erro no zip.js: ${stderr}`);
            }
